...
Features:
- Introduced [`IF NOT EXISTS` clause on `DEFINE` statements](https://surrealdb.com/docs/surrealdb/surrealql/statements/define/).
- Implemented [`IF EXISTS` for additional `REMOVE` statements](https://surrealdb.com/docs/surrealdb/surrealql/statements/remove#using-if-exists-clause-since-130).
...
`DEFINE` statements now allow you to include an `IF NOT EXISTS` clause which ensures that the define statement is only run if the resource doesn't already exist. If it exists, you will get an error. Learn more about this in [the documentation](https://surrealdb.com/docs/surrealdb/surrealql/statements/define/)
...
The KNN operator now supports a new syntax using `<|` and `|>` instead of `<` and `>`. Learn more about this in [the documentation](https://surrealdb.com/docs/surrealdb/surrealql/operators#knn-since-130)
